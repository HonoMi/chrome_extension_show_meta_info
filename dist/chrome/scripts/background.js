...
// import 'chromereload/devonly'
{
    ...
    var URLHASH_TO_QUERYWORDS_DICT = {};

    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        if (request.method == "storeQueryWords") {
            var googleQueryWords = request["googleQueryWords"];
            var urlsAll = Object.keys(request["urls"]).map(function (key) {
                return request["urls"][key].toString();
            }).map(function (url) {
                return decodeURIComponent(url);
            });
            var urls = urlsAll.filter(function (url) {
                return !url.match(/^#.*/);
            });;
            var urlHashes = urls.map(function (url) {
                return util.url2hash(url);
            });
            urlHashes.forEach(function (hash) {
                return URLHASH_TO_QUERYWORDS_DICT[hash] = googleQueryWords;
            });
            sendResponse({});
        } else if (request.method == "getQueryWords") {
            var url = decodeURIComponent(request["url"]);
            var urlHash = util.url2hash(url);
